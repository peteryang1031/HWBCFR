...
from ._sinkhorn import sinkhorn, sinkhorn_stabilized
...
def free_support_sinkhorn_barycenter_unbalanced(measures_locations, measures_weights, X_init, reg, reg_m, b=None, scale=None, weights=None,
    ...
    while (displacement_square_norm > stopThr and iter_count < numItermax):
        ...
        for (measure_locations_i, measure_weights_i, weight_i) in zip(measures_locations, measures_weights, weights):
            ...
            M_i = (M_i.sub_(torch.min(M_i))).div_((torch.max(M_i).sub_(torch.min(M_i))))

            T_i = sinkhorn_unbalanced(b, measure_weights_i, M_i, reg=reg, reg_m=reg_m,
                           numItermax=numInnerItermax, stopThr=1e-5, **kwargs)
...
def free_support_sinkhorn_barycenter(measures_locations, measures_weights, X_init, reg, scale, b=None, weights=None,
                                     numItermax=100, numInnerItermax=1000, stopThr=1e-7, verbose=False, log=None,
                                     **kwargs):
    ...
    while (displacement_square_norm > stopThr and iter_count < numItermax):
        ...
        for (measure_locations_i, measure_weights_i, weight_i) in zip(measures_locations, measures_weights, weights):
            X = X
            measure_locations_i =  measure_locations_i
            M_i = dist(X, measure_locations_i)
